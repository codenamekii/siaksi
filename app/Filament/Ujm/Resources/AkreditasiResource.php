...
// 1. app/Filament/Ujm/Resources/AkreditasiResource.php
...
class AkreditasiResource extends Resource
{
        ...
        Forms\Components\Section::make('Informasi Akreditasi')
          ->schema([
            ...
            Forms\Components\TextInput::make('nilai_akreditasi')
              ...
              ->maxValue(400)
              ->minValue(0)
              ->placeholder('Contoh: 361')
              ->helperText('Nilai akreditasi (jika ada)')
              ->suffix('/ 400'),
            Forms\Components\TextInput::make('nomor_sk')
              ->label('Nomor SK')
              ->required()
              ->maxLength(255)
              ->placeholder('Contoh: 123/SK/BAN-PT/Ak/S/V/2024'),
          ])->columns(2),
        ...
        Forms\Components\Section::make('Periode Akreditasi')
          ->schema([
            Forms\Components\DatePicker::make('tanggal_akreditasi')
              ...
              ->afterStateUpdated(function (Get $get, Forms\Set $set) {
                if ($get('tanggal_akreditasi')) {
                  $set(
                    'tanggal_berakhir',
                    \Carbon\Carbon::parse($get('tanggal_akreditasi'))->addYears(5)->format('Y-m-d')
                  );
                }
              }),
            Forms\Components\DatePicker::make('tanggal_berakhir')
              ->required()
              ->native(false)
              ->after('tanggal_akreditasi')
              ->helperText('Biasanya 5 tahun setelah tanggal akreditasi'),
            Forms\Components\TextInput::make('tahun_akreditasi')
              ->disabled()
              ->dehydrated(false)
              ->default(fn(Get $get) => $get('tanggal_akreditasi')
                ? \Carbon\Carbon::parse($get('tanggal_akreditasi'))->year
                : null),
          ])->columns(3),
        ...
        Forms\Components\Section::make('Status')
          ->schema([
            Forms\Components\Toggle::make('is_active')
              ...
              ->default(true)
              ->helperText('Hanya satu akreditasi yang bisa aktif pada satu waktu')
              ->reactive()
              ->afterStateUpdated(function ($state, $record) {
                if ($state && $record) {
                  // Set akreditasi lain menjadi tidak aktif
                  AkreditasiProdi::where('program_studi_id', $record->program_studi_id)
                    ->where('id', '!=', $record->id)
  ...
  public static function table(Table $table): Table
  {
    return $table
      ->columns([
        Tables\Columns\TextColumn::make('lembaga_akreditasi')
          ->label('Lembaga')
          ->searchable(),
        ...
        Tables\Columns\TextColumn::make('nomor_sk')
          ...
          ->searchable()
          ->limit(30)
          ->tooltip(fn($record) => $record->nomor_sk),
        Tables\Columns\TextColumn::make('tanggal_akreditasi')
          ->label('Tanggal')
          ->date('d M Y')
          ->sortable(),
        Tables\Columns\TextColumn::make('tanggal_berakhir')
          ...
          ->color(
            fn($record) =>
            $record->tanggal_berakhir->isPast() ? 'danger' : ($record->tanggal_berakhir->diffInMonths(now()) <= 6 ? 'warning' : 'gray')
          ),
        Tables\Columns\TextColumn::make('sisa_masa_berlaku')
          ...
          ->color(
            fn($record) =>
            $record->tanggal_berakhir->isPast() ? 'danger' : ($record->tanggal_berakhir->diffInMonths(now()) <= 6 ? 'warning' : 'success')
          ),
      ...
      ->filters([
        Tables\Filters\SelectFilter::make('lembaga_akreditasi')
          ->options([
            'BAN-PT' => 'BAN-PT',
            'LAM-Infokom' => 'LAM-Infokom',
            'LAM-Teknik' => 'LAM-Teknik',
          ]),
        Tables\Filters\SelectFilter::make('status_akreditasi')
          ->options([
            'Unggul' => 'Unggul',
            'Baik Sekali' => 'Baik Sekali',
            'Baik' => 'Baik',
