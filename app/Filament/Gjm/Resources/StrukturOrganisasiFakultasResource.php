        ...
        Forms\Components\Section::make('Upload Struktur Organisasi')
          ...
          ->schema([
            ...
            Forms\Components\Textarea::make('deskripsi')
              ...
              ->rows(3)
              ->maxLength(500)
              ->helperText('Jelaskan periode atau informasi tambahan'),
            ...
            Forms\Components\Toggle::make('is_active')
              ->label('Status Aktif')
              ->default(true)
              ->helperText('Nonaktifkan jika struktur sudah tidak berlaku'),

            Forms\Components\Hidden::make('level')
              ->default('fakultas'),

            Forms\Components\Hidden::make('fakultas_id')
              ->default(fn() => Auth::user()->fakultas_id ?? 1),
          ])
          ->columns(1),
      ]);
  }
    ...
    return $table
      ...
      ->actions([
        Tables\Actions\Action::make('view')
          ...
          ->icon('heroicon-o-eye')
          ->color('info')
          ->modalHeading(fn(StrukturOrganisasi $record) => $record->nama)
          ->modalContent(fn(StrukturOrganisasi $record) => view('filament.gjm.modals.view-struktur', compact('record')))
          ->modalWidth('7xl'),
        ...
        Tables\Actions\Action::make('download')
          ...
          ->color('success')
          ->url(fn(StrukturOrganisasi $record) => asset('storage/' . $record->file_path))
          ->openUrlInNewTab(),
