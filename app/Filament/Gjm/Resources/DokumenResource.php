...
// 1. app/Filament/Gjm/Resources/DokumenResource.php
...
class DokumenResource extends Resource
{
        ...
        return $form->schema([
            ...
            Forms\Components\Section::make('Upload Dokumen')
                ->schema([
                    ...
                    Forms\Components\TextInput::make('url')->label('URL Dokumen')->url()->helperText('Masukkan URL lengkap dokumen')->visible(fn(Get $get) => $get('tipe') === 'url')->required(fn(Get $get) => $get('tipe') === 'url'),
                ])
                ->columns(1),

            Forms\Components\Section::make('Klasifikasi')
                ->schema([Forms\Components\TextInput::make('kriteria')->helperText('Contoh: Tata Kelola, Pendidikan, Penelitian'), Forms\Components\TextInput::make('sub_kriteria')->label('Sub Kriteria')->helperText('Contoh: Sistem Penjaminan Mutu, Kurikulum'), Forms\Components\Textarea::make('catatan')->rows(2)->columnSpanFull()])
                ->columns(2),

            Forms\Components\Section::make('Pengaturan')
                ->schema([
                    Forms\Components\Toggle::make('is_visible_to_asesor')->label('Tampilkan ke Asesor')->helperText('Dokumen dapat dilihat oleh asesor')->default(false),
                    ...
                    Forms\Components\Hidden::make('fakultas_id')->default(1), // Default fakultas ID
                ])
                ->columns(2),
        ]);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->actions([Tables\Actions\Action::make('download')->label('Download')->icon('heroicon-o-arrow-down-tray')->color('success')->url(fn(Dokumen $record): string => $record->tipe === 'file' ? asset('storage/' . $record->path) : $record->url)->openUrlInNewTab(), Tables\Actions\EditAction::make(), Tables\Actions\DeleteAction::make()])
            ->bulkActions([
